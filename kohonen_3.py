...
class Neuron:
    ...
    def compute(self, x):
        ...
        @type x: numpy array
        """
        # Attention il faut bien renvoyer la distance entre le poids et l'entrée et pas 0/1 suivant si le neurone est
        #  gagnant ou pas comme dans le TP (la détermination du neurone gagnant se fait ailleurs dans le code)
    ...
    def learn(self, eta, sigma, posxbmu, posybmu, x):
        ...
        # Hexagonnal grid
        delta = eta * (exp(-((self.hexDistance(posxbmu, posybmu) ** 2) / (2 * sigma * sigma)))) * (x - self.weights)
        self.weights[:] = self.weights[:] + delta

    def hexDistance(self, px, py):
        """
        Calcule la distance entre deux neurones, dans un repère hexagonale
        :param px: position x
        :param py: position y
        :return: distance entre les deux neurones
        """
        x0 = self.posx - math.floor(self.posy / 2)
        y0 = self.posy
        x1 = px - math.floor(py / 2)
        y1 = py
        dx = x1 - x0
        dy = y1 - y0
...
# -----------------------------------------------------------------------------
def percent(current, vmax, str):
    print(str, " : ", current, "/", vmax, " (", 100.0 * current / vmax, "%)")

if __name__ == '__main__':
    # Création d'un réseau avec une entrée (2, 1) et une carte (10, 10)
    # (TODO ATTENTION METTRE À JOUR LA TAILLE DE L'ENTRÉE POUR LES DONNÉES MNIST)
    network = SOM((28, 28), (50, 50))
